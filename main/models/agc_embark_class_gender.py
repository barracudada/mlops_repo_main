...
with open(
    "test.csv", "rb"
) as f:  # Handel's the opening and closing of the file for us.
...
cop_open_file = open(
    "results_embarkclassgendermodel.csv", "wb"
)  # Creates a csv file with write privileges ("wb") called
...
with open(
    ...
    for row in csv_file:
        surived_pred = (
            C
            + (female * int(data[x][1]))
            + (U_Class * int(data[x][0]))
            + (embarked_c * int(data[x][2]))
        )

        if survived_prediction > 0.50000:
            pred_val = 1
        else:
            pred_val = 0
