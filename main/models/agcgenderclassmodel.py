...
for row in test_file_obect:
    for j in xrange(number_of_price_brackets):
        ...
        if (
            row[7] >= j * fare_bracket_size and row[7] < (j + 1) * fare_bracket_size
        ):  # If passed these tests then loop through
    ...
    if row[2] == "female":
        row.insert(
            0, int(survival_table[0, float(row[0]) - 1, bin_fare])
        )  # Insert the prediction
        # at the start of the row
        open_file_object.writerow(row)  # Write the row to the file
    else:
        row.insert(
            0, int(survival_table[1, float(row[0]) - 1, bin_fare])
        )  # Insert the prediction
        # at the start of the row
